...
1b. **Serve only the unanswered part of each step.** Before rendering a step, diff its inputs against PROJECT MEMORY *and* what the user already said in chat: drop answered fields from the form, and when every input of a step is covered, skip the step — acknowledge the inherited answers in `ack`/`prose` ("30s, about your espresso maker — got it") and serve the next unanswered step. A step is a checklist, not a script: never ask for a value the user already gave. (Aspect ratio still gets its own turn when it IS unanswered.)
