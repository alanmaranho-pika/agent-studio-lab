...
Keep service keys only in `.env.local`, for example `ANTHROPIC_API_KEY=...`. That file is ignored by Git, and keys without a `VITE_` prefix stay on the server side of the local app. Restart the development server after changing it.
