...
-- this table stores each user's live edits so the in-app skill editor works
-- on both localhost and read-only serverless deployments (Vercel, Workers).
...
CREATE POLICY "Users can delete own skill playbook overrides"
  ...
  USING ((SELECT auth.uid()) = user_id);
CREATE TRIGGER trg_skill_playbook_overrides_touch BEFORE UPDATE ON public.skill_playbook_overrides
  FOR EACH ROW EXECUTE FUNCTION public.touch_updated_at();