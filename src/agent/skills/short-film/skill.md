...
**Required result:** once specs, cast, audio, and the storyboard are set, this skill ALWAYS lands on a `BLK_TIMELINE` of the shots (Step 5) — never a written recap, `list`, or prose summary of the project. The timeline IS the review surface; a summary is not an acceptable substitute.
...
## Step 5 — Timeline preview (REQUIRED)
...
- The timeline draws straight from `project.scenes` (patched in Step 4), so it shows the shots even before any video is rendered — pending shots appear as placeholder clips.
